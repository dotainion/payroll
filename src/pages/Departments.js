...
export const Departments = () =>{
    ...
    const onDelete = (e) =>{
        ...
        api.department.delete(id).then((response)=>{
            parent.remove();
            toast.success('Bank', 'Created');
        }).catch((error)=>{
            console.log(error);
    ...
    const onEdit = (e) =>{
        ...
        api.department.edit(id, name).then((response)=>{
    ...
    return(
        <div className="page profile text-nowrap">
            <div className="border-bottom p-2 fw-bold bg-secondary text-white mt-2">Departments</div>
            <div className="my-3">
                <button ref={buttonRef} className="btn btn-info btn-sm">New Department +</button>
            </div>
            <DepartmentCard ref={inputContainerRef} nameRef={nameRef} closeRef={closeRef} onSave={onSave}/>
            <hr></hr>
            <div ref={scrollRef} className="overflow-auto">
                {departments.map((bank, key)=>(
                    <div onClick={(e)=>e.stopPropagation()} className="my-2" key={key}>
                        ...
                        <div className="border border-info p-1" data-editable="" style={{display: 'none'}}>
                            <DepartmentCard onSave={onEdit} onClose={onCloseAllEdit} data={bank} show/>
                        </div>
                    </div>
                ))}
            </div>
...
const DepartmentCard = forwardRef(({nameRef, closeRef, onSave, onClose, data, show}, ref)=>{
    return(
        <div ref={ref} style={{display: show ? '' : 'none'}}>
            <div className="allowance-row border m-3">
                <div className="input-group">
                    <span className="input-group-text"><HiBuildingOffice2/></span>
                    <input ref={nameRef} className="form-control shadow-none" name="name" placeholder="Department Name" defaultValue={data?.attributes?.name}/>
                </div>
            </div>
            <div className="my-3">
